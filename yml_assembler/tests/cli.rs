...
fn check_generated_file_ok(root: &str, output: &str, file: &str) {
    //read assembles
    let assembled_file_path = PathBuf::from(root)
        .join(output)
        .join(format!("{}.yml", file));
...
fn delete_output_folder(root: &str, output: &str) {
...
fn it_should_run_cli() {
    ...
    check_generated_file_ok(&root, &output, &file);
    delete_output_folder(&root, &output);
}

#[test]
#[serial]
fn it_should_run_cli_with_relative_root() {
    let (mut cmd, root, output, file, _) = run_cli("./tests/yml_test_files");
...
fn it_should_generate_file_with_ordered_entries() {
    let (mut cmd, root, output, _, _) = run_cli("./tests/yml_test_files");
